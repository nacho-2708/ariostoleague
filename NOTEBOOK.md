...
- **2026-07-01 — Propagación Broadcast completa: Fundación + Standings + Fixtures + Managers + Stats** — Reskin del contenido de cada sección a superficie oscura (solo estilos, lógica/datos intactos). Standings fue el stress test (tabla densa) y aguantó. El **recetario de reskin** (claro→oscuro, cuándo lime vs blue, tipografía, primitivos, charts) quedó documentado en la skill **`ariosto-ui`** (`.claude/skills/ariosto-ui/`) — leerla antes de reskinear. Fundación+Standings+Fixtures+Managers se mergearon a `main` vía PR #1. **Stats** (`(shell)/stats/**` + componentes `src/components/stats/**`) cerró el epic en PR #2, rama `2026-07-01-stats-broadcast` — el hallazgo ahí fue que las rutas casi no tenían tokens claros; el grueso vivía en los componentes compartidos (leaderboards, compare, records, subnav), no en las páginas.
...
- **recharts + SSR: `ResponsiveContainer` no pinta las barras reales en el HTML del servidor.** Necesita medir el ancho del contenedor en el browser (usa `window`/`ResizeObserver`), así que un `curl` a una página con un chart de recharts (ej. `/stats/compare`) devuelve el wrapper vacío — sin colores de `fill`, sin geometría real de las barras — aunque el chart funcione perfecto una vez hidratado en un navegador real. **No sirve como verificación de que el chart "se ve bien"**, solo confirma que la página no rompe (sin excepciones, sin `NaN`, build/TS limpios). Para verificar de verdad el render de un chart hace falta un navegador real (o una tool de screenshot, que este entorno no tiene) — pedirle confirmación visual a Nacho. _Anotado: 2026-07-01, reskin de Stats/Comparar._
